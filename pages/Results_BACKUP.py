...
# --- CREATIVE FEATURE: CURRENCY CONVERTER + SIMPLE MODE + DEBUG MODE ---
with st.sidebar:
    st.header("🌍 Settings")
    ...
    # Phase 4: Debug Mode (Developer/Testing)
    st.markdown("---")
    st.header("🔧 Debug")
    # Check query parameter first, then session state
    debug_mode = st.checkbox(
        "Show debug info",
        value=st.query_params.get("debug", "0") == "1" or st.session_state.get("debug_mode", False),
...
# Worst/Base/Best Case (62. CFO) - Phase 4: Use cost_scenarios if available
if 'cost_scenarios' in result:
    # Use new engine's cost scenarios
    cost_scenarios = result['cost_scenarios']
    base_cost = cost_scenarios.get('base', total_landed_cost)
    best_cost = cost_scenarios.get('best', base_cost * 0.90)
    worst_cost = cost_scenarios.get('worst', base_cost * 1.15)
else:
...
tab_costs, tab_risks, tab_lead_time, tab_data_quality = st.tabs([
    "💰 Cost Breakdown", 
    "⚠️ Risk Analysis", 
    "⏰ Lead Time",
    "📊 Data Quality"
...
with tab_risks:
    ...
    # Phase 4: Show new risk_scores if available
    if 'risk_scores' in result:
        risk_scores = result['risk_scores']
        
        # Success probability and overall risk score
        success_prob = risk_scores.get('success_probability', 0)
        ...
        for risk_name, risk_value in sub_risk_data.items():
            risk_pct = risk_value / 100.0
            if risk_pct < 0.3:
            ...
            st.markdown(f"""
                <div style="margin-bottom: 0.75rem;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                        <span style="color: #e2e8f0;">{risk_name}</span>
                        <span style="color: {risk_color}; font-weight: 600;">{risk_value:.1f}/100</span>
                    </div>
                    <div style="width: 100%; height: 8px; background: rgba(51, 65, 85, 0.5); border-radius: 4px; overflow: hidden;">
                        <div style="width: {risk_pct * 100}%; height: 100%; background: {risk_color}; transition: width 0.3s;"></div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
...
with tab_data_quality:
    st.markdown("<h4>Data Quality & Sources</h4>", unsafe_allow_html=True)
    st.markdown('<div class="glass-container">', unsafe_allow_html=True)
    
    # Phase 4: Show data quality information
    if 'data_quality' in result:
        ...
        if len(used_fallbacks) == 0:
            st.success("✅ All data retrieved from primary sources (CSV/Supabase)")
            data_source = "Primary (CSV/Supabase)"
        else:
            st.warning(f"⚠️ Some data used fallback estimates: {', '.join(used_fallbacks)}")
            data_source = f"Mixed (Fallback: {', '.join(used_fallbacks)})"
        ...
        st.markdown(f"""
            <div style="margin-top: 1rem; padding: 1rem; background: rgba(30, 41, 59, 0.5); border-radius: 8px;">
                <div style="margin-bottom: 0.5rem;">
                    ...
                    <span style="color: #94a3b8;">{data_source}</span>
                </div>
                <div>
                    <strong style="color: #e2e8f0;">Reference Transactions:</strong> 
                    <span style="color: #94a3b8;">{ref_count} similar transactions found</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
        ...
        if used_fallbacks:
            st.markdown("""
                <div style="margin-top: 1rem; padding: 1rem; background: rgba(245, 158, 11, 0.1); border-left: 4px solid #f59e0b; border-radius: 8px;">
                    <p style="color: #fbbf24; font-size: 0.9rem; margin: 0;">
                        <strong>Note:</strong> Fallback values are estimates based on industry averages. 
                        For more accurate analysis, consider adding actual market data to CSV files or Supabase.
                    </p>
                </div>
            """, unsafe_allow_html=True)
    else:
        st.info("Data quality information not available in this analysis.")
    
    st.markdown("</div>", unsafe_allow_html=True)

# Phase 4: Debug Mode - Show raw JSON
if st.session_state.get("debug_mode", False):
    st.markdown("---")
    st.markdown("### 🔧 Debug Information")
    
    with st.expander("ShipmentSpec (Parsed Input)", expanded=False):
        if 'shipment_spec' in st.session_state:
            st.json(st.session_state['shipment_spec'])
        else:
            st.info("ShipmentSpec not found in session state")
    
    with st.expander("AnalysisResult (Full JSON)", expanded=False):
        st.json(result)
